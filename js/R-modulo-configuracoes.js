async function performHardReset() {
    const savedLicenseKey = localStorage.getItem('ap_license_key');

    Object.values(KEYS).forEach(key => localStorage.removeItem(key));
    
    localStorage.removeItem('ap_onboarding_complete');
    localStorage.removeItem('ap_onboarding_progress');
    localStorage.removeItem('ap_data_version');
    localStorage.removeItem('ap_last_auto_backup_timestamp');
    localStorage.removeItem('ap_unlock_level');
    localStorage.removeItem('ap_tutorial_dismissed');
    
    if (savedLicenseKey) {
        localStorage.setItem('ap_license_key', savedLicenseKey);
    }
...
function loadConfigForm() {
    ...
    const configNomeInput = document.getElementById("configNome");
...
async function exportAllData(isAutoBackup = false) {
    ...
    try {
        ...
        triggerDownload(dataBlob, `${prefix}_aula_pro_${dateString}_${timeString}.json`);

        success = true;

        if (isAutoBackup) {
            localStorage.setItem('ap_last_auto_backup_timestamp', Date.now().toString());
    ...
    fileInput.onchange = async (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // Validação das chaves do Aula Pro
                const requiredKeys = ['estruturas', 'materias', 'professores', 'turmas', 'horarios', 'config'];
                ...
                const { confirmed } = await showConfirm({
                    ...
                    message: "<strong>ATENÇÃO:</strong> Isto irá substituir TODOS os seus dados atuais (matérias, professores, turmas, etc.) pelos dados do arquivo. Esta ação é IRREVERSÍVEL.<br><br><strong>Deseja continuar?</strong>",
                ...
                if (confirmed) {
                    ...
                    localStorage.setItem('ap_data_version', "1.0");
                    localStorage.removeItem('ap_last_auto_backup_timestamp');
...
function exibirTermosDeUso(requireScrollableConfirm = false) {
    const termosDeUsoHTML = `
        <div style="font-size: 0.9rem; line-height: 1.6;">
            <h3>🔒 AULA PRO — TERMOS DE USO E LICENÇA DE SOFTWARE (VENDA ÚNICA)</h3>
            ...
            <p>Ao utilizar o software <strong>Aula Pro</strong> (“Software”), o usuário (“Usuário”) concorda integralmente com estes <strong>Termos de Uso e Licença</strong> (“Termos”). Se você não concorda com qualquer parte destes Termos, <strong>não deve utilizar o Software</strong>.</p>
            <hr>

            <h4>2. Natureza do Software e Armazenamento de Dados</h4>
            <p><strong>2.1.</strong> O Aula Pro é uma aplicação que opera <strong>exclusivamente neste programa, instalado no seu computador</strong>.</p>
            <p><strong>2.2.</strong> <strong>Todos os dados inseridos</strong> (como informações de professores, matérias, turmas e grades horárias) são <strong>armazenados localmente</strong> no dispositivo do Usuário, por meio do <strong>armazenamento local do programa</strong>.</p>
            ...
            <p><strong>3.4. Propriedade Intelectual:</strong> O Aula Pro é protegido pelas leis de direitos autorais e propriedade intelectual. Nenhum direito, título ou interesse é transferido ao usuário além do direito limitado de uso descrito nestes Termos.</p>
            <p><strong>3.5. Uso Indevido:</strong> O uso, cópia ou distribuição não autorizada do Aula Pro, no todo ou em parte, poderá resultar em medidas legais e indenizações previstas pela Lei nº 9.609/98 (Lei de Software).</p>
            <hr>
            ...
            <p><strong>5.2. Conformidade e Resultados:</strong> As grades e informações geradas são baseadas nas regras inseridas pelo Usuário. É de sua exclusiva responsabilidade garantir que os horários estejam <strong>em conformidade com as diretrizes pedagógicas e regulamentações da instituição</strong>. O Software é uma ferramenta de auxílio e está sujeito a erros. É de responsabilidade do Usuário revisar e validar todas as grades geradas.</p>
            <hr>
            ...
            <p>O Software é fornecido <strong>“COMO ESTÁ” (“AS IS”)</strong>, sem garantias de qualquer tipo, expressas ou implícitas. O Licenciante <strong>não garante</strong> que o Software atenderá a requisitos específicos, nem que funcionará de forma ininterrupta, livre de erros ou segura.</p>
            <hr>

            <h4>8. Limitação de Responsabilidade</h4>
            <p>Em nenhuma circunstância o Licenciante será responsável por <strong>quaisquer danos diretos, indiretos, acidentais, consequenciais ou punitivos</strong>, incluindo perda de dados, interrupção de atividades ou outras perdas resultantes do uso ou incapacidade de uso do Software.</p>
            <hr>
            ...
            <p>Estes Termos são regidos pelas <strong>leis brasileiras</strong>. Fica eleito o foro da <strong>comarca de Pradópolis - SP</strong> como competente para resolver quaisquer controvérsias.</p>
            <hr>
    ...
    if (requireScrollableConfirm) {
        return showScrollableConfirmModal({
            title: "Termos de Uso do Aula Pro",
...
function exibirPoliticaDePrivacidade(requireScrollableConfirm = false) {
     const politicaHTML = `
        <div style="font-size: 0.9rem; line-height: 1.6;">
            <h3>🛡️ POLÍTICA DE PRIVACIDADE — AULA PRO</h3>
            <p><strong>Última atualização:</strong> 15/10/2025<br>
            <strong>Contato:</strong> escalafacil.contato@gmail.com</p>
            <hr>

            <h4>1. Princípio Fundamental: Seus Dados São Apenas Seus</h4>
            <p>O <strong>Aula Pro</strong> foi desenvolvido com o princípio de <strong>privacidade total</strong>:</p>
            <ul>
            ...
            <p>O Aula Pro <strong>não utiliza cookies de rastreamento, pixels, nem serviços de terceiros</strong> (como Google Analytics ou APIs externas).</p>
            <hr>
            ...
            <ul>
                <li>O Aula Pro não coleta <strong>nenhum dado</strong>.</li>
    ...
    if (requireScrollableConfirm) {
        return showScrollableConfirmModal({
            title: "Política de Privacidade do Aula Pro",
...
function exibirAtalhosDeTeclado() {
    const shortcuts = [
        { keys: ['↑', '↓', '←', '→'], desc: 'Navegam pela grade de horários.' },
        { keys: ['Q', 'E'], desc: 'Trocam o professor focado na Barra de Ferramentas.' },
        { keys: ['Enter'], desc: 'Abre o menu de alocação para a célula selecionada.' },
        { keys: ['Delete', 'Backspace'], desc: 'Removem a aula da célula focada.' },
    ];
...
function initConfiguracoesPage() {
    ...
    const tabs = Array.from(page.querySelectorAll("#config-tabs .painel-tab-btn"));
    const panes = Array.from(page.querySelectorAll("#config-content .config-pane"));
    ...
    const termsCard = document.getElementById("config-terms-card");
    if(termsCard) termsCard.onclick = () => exibirTermosDeUso();

    const privacyCard = document.getElementById("config-privacy-card");
    if(privacyCard) privacyCard.onclick = () => exibirPoliticaDePrivacidade();

    const shortcutsCard = document.getElementById("config-shortcuts-card");
    ...
    if(btnReset){
        btnReset.onclick = async () => {
            const { confirmed } = await showConfirm({
                ...
                message: "Esta ação é IRREVERSÍVEL. Todas as estruturas, matérias, professores, turmas e grades salvas serão permanentemente excluídos deste programa.<br><br><strong>Tem certeza absoluta?</strong>",
